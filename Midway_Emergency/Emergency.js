...
const json = (() => {
    ...
    $.ajax({
        'async': false,
        'global': false,
        'url': "./csvjson.json", //Json file location
        'dataType': "json",
...
function initMap() {
    ...
    $.each(waterfalls, function(key, data) {
        ...
        var details = `<div class="marker"><strong><h3>${data.Address}</h3></strong> 
                        Given Name: <span class="number">${data.GivenName}</span> FirstName: <span class="number">${data.FirstName}</span> LastName: <span class="number">${data.LastName}</span>
                        <br> Address: <span class="number">${data.Address}</span>
